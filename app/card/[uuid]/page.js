  ...
  const handleClaim = async (cardUuid) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        // Redirect to onboarding page after successful claim
        router.push(`/onboarding/${cardUuid}`);
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-2xl w-full">
          <ErrorPage type={errorType} message={error} onRetry={loadCard} />
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 sm:py-8">
      <div className="max-w-4xl mx-auto px-0 sm:px-6 lg:px-8">
        <div className="space-y-6 sm:space-y-8">
          <div className="max-w-2xl mx-auto">
            {cardData.claimed ? (
              ...
              <div className="bg-white rounded-lg shadow-sm border p-8 text-center">
                ...
                <p className="text-gray-600 mb-6">{cardData.message}</p>
          ...
          {user && !cardData.claimed && (
            <div className="flex justify-center">
              ...
              >
                ...
                <span>{claiming ? 'Claiming...' : 'Claim This Card'}</span>
              </Button>
            </div>
          )}
          ...
          {user &&
            cardData.claimed &&
            ((fullCardData?.owner?.id && fullCardData.owner.id === user.id) ||
              (fullCardData?.ownerId && fullCardData.ownerId === user.id)) && (
              <div className="flex justify-center">
